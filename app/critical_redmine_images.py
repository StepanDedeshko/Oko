...
CriticalRedmineCreateDialog = None
...
def build_critical_redmine_description(context: dict) -> str:
    """Build a user-facing description that embeds uploaded attachment names."""
    item = context["item"]
...
def install_critical_redmine_images() -> bool:
    """Install the critical image flow only after QApplication exists."""
    global CriticalRedmineCreateDialog
    ...
    if QApplication.instance() is None:
        raise RuntimeError("install_critical_redmine_images() requires an active QApplication")
    ...
    class RuntimeCriticalRedmineCreateDialog(RedmineCreateDialog):
        """Redmine form that uploads chart PNGs before enabling submit."""
        ...
        def _submit_enabled_script(enabled: bool) -> str:
            ...
            marker = json.dumps("0" if enabled else "1")
            return f"""
(function() {{
...
"""
    ...
    CriticalRedmineCreateDialog = RuntimeCriticalRedmineCreateDialog
    original_analysis_complete = cls._critical_analysis_complete
    original_finish_flow = cls._finish_critical_flow

    def cleanup_chart_view(self) -> None:
    ...
    def decode_chart_result(result) -> dict:
    ...
    def analysis_complete_with_images(self, analysis, token: int) -> None:
        context = self._critical_context
        if not context or token != context.get("token"):
            return
        if context.get("chart_attachments_ready"):
            return original_analysis_complete(self, analysis, token)
    ...
    def capture_next_chart(self, token: int) -> None:
        context = self._critical_context
        if not context or token != context.get("token"):
            return
        ...
        if index >= len(urls):
            ...
            return original_analysis_complete(self, context["analysis"], token)
    ...
    def chart_loaded(self, ok: bool, token: int, index: int) -> None:
        context = self._critical_context
    ...
    def try_extract_chart(self, token: int, index: int) -> None:
        context = self._critical_context
        if not context or token != context.get("token") or index != context.get("chart_capture_index"):
            return
        view = getattr(self, "_critical_chart_view", None)
    ...
    def chart_result(self, result, token: int, index: int) -> None:
        context = self._critical_context
        if not context or token != context.get("token") or index != context.get("chart_capture_index"):
            return
        payload = decode_chart_result(result)
    ...
    def chart_capture_failed(self, token: int, reason: str) -> None:
        context = self._critical_context
        if not context or token != context.get("token"):
            return
        self.logger.warning(
    ...
    def open_redmine_dialog_with_images(self, token: int) -> None:
        context = self._critical_context
        if not context or token != context.get("token"):
            return
        attachments = list(context.get("chart_attachments") or [])
        ...
        try:
            ...
            profile = self.view.page().profile() if self.view is not None and self.view.page() is not None else None
            dialog = RuntimeCriticalRedmineCreateDialog(
                profile,
                redmine_url,
    ...
    def finish_flow_with_chart_cleanup(self, token=None) -> None:
        self._cleanup_critical_chart_view()
        return original_finish_flow(self, token)

    cls._cleanup_critical_chart_view = cleanup_chart_view
    cls._critical_capture_next_chart = capture_next_chart
    cls._critical_chart_loaded = chart_loaded
    cls._critical_try_extract_chart = try_extract_chart
    cls._critical_chart_result = chart_result
    cls._critical_chart_capture_failed = chart_capture_failed
    cls._critical_analysis_complete = analysis_complete_with_images
    cls._open_critical_redmine_dialog = open_redmine_dialog_with_images
    cls._finish_critical_flow = finish_flow_with_chart_cleanup
    cls._critical_redmine_images_dialog_class = RuntimeCriticalRedmineCreateDialog
