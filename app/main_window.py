...
creation of the Live Zabbix page and the critical-task integration are
specialized here; startup/release/theme code remains outside this feature.
"""
...
from app.live_zabbix_widget import (
    ...
    LiveZabbixMonitorWidget as _StandardLiveZabbixMonitorWidget,
    RedmineCreateDialog,
)
...
CRITICAL_CHART_RETRY_DELAYS_MS = (250, 650, 1_200, 2_000, 3_200)
CRITICAL_ATTACHMENT_POLL_MS = 500
CRITICAL_ATTACHMENT_MAX_POLLS = 36

CRITICAL_CHART_EXTRACTION_SCRIPT = r"""
(function() {
...
def _critical_attachment_filename(item, index: int) -> str:
    event_key = getattr(item, "event_id", "") or getattr(item, "key", "") or "event"
    suffix = "main" if index == 0 else "all_operations"
    return f"oko_critical_{_safe_attachment_stem(event_key)}_{suffix}.png"


def _critical_redmine_description(context: dict) -> str:
    """Build the operator-facing text using uploaded attachment filenames."""
    item = context["item"]
    ...
    # The first scenario only needs its main graph. Slow-share scenarios retain
    # the useful calculated result, but never expose implementation instructions.
...
class CriticalRedmineCreateDialog(RedmineCreateDialog):
    """Upload critical chart PNGs before allowing Redmine issue creation."""
    ...
    def __init__(
        ...
        self._attachment_started = False
    ...
    @staticmethod
    def _submit_lock_script(locked: bool) -> str:
        disabled = "true" if locked else "false"
        marker = json.dumps("1" if locked else "0")
        return f"""
(function() {{
  ...
  buttons.forEach(function(button) {{
    ...
    button.setAttribute('data-oko-critical-lock', {marker});
  }});
...
"""
    ...
    @staticmethod
    def _attachment_injection_script(attachments) -> str:
        payload = json.dumps(
            [
                {
...
"""
    ...
    @staticmethod
    def _attachment_status_script(filenames) -> str:
...
(function() {{
  ...
  var errors = Array.from(document.querySelectorAll('.flash.error, .errorExplanation, .file-upload-error'))
...
"""
    ...
    def _set_submit_locked(self, locked: bool) -> None:
        page = self.view.page() if self.view is not None else None
        if page is not None:
            run_javascript_if_alive(page, self._submit_lock_script(locked))

    def _on_create_loaded(self, ok):
        self._set_submit_locked(True)
        super()._on_create_loaded(ok)
    ...
    def _try_attachment_injection(self) -> None:
        ...
        run_javascript_if_alive(
            page,
            self._attachment_injection_script(self.pending_attachments),
    ...
    def _poll_attachment_status(self) -> None:
        ...
        if page is None:
            return
        self._attachment_polls += 1
        filenames = [item.get("filename") for item in self.pending_attachments]
        run_javascript_if_alive(
            page,
            self._attachment_status_script(filenames),
    ...
    def _on_attachment_status_result(self, result) -> None:
        ...
        # Standard Redmine creates upload tokens. Some installations keep files
        # on the issue form itself, so a stable selected-file list is accepted too.
        ...
        if uploaded or direct_form_ready:
            self._attachment_ready = True
            get_logger().info(
                "Critical Redmine attachments ready: expected=%s tokens=%s selected=%s",
                expected,
                token_count,
                selected_count,
            )
        ...
        if error_text:
            self._attachment_failed(error_text)
            return
        if self._attachment_polls >= CRITICAL_ATTACHMENT_MAX_POLLS:
    ...
    def _attachment_failed(self, reason: str) -> None:
        ...
        self.status_label.setText(
            ...
            "чтобы она не ушла без обязательных изображений. Закройте окно и повторите попытку."
        )
        self.status_label.setVisible(True)
        self._set_submit_locked(True)
...
class CriticalLiveZabbixMonitorWidget(_CriticalLiveZabbixMonitorWidget):
    ...
    @staticmethod
    def _decode_chart_result(result):
    ...
    def _cleanup_critical_chart_view(self) -> None:
        view = self._critical_chart_view
        self._critical_chart_view = None
        if view is not None:
            safe_delete_web_view(
                view,
                logger=self.logger,
                context="CriticalLiveZabbix chart capture",
            )

    def _critical_analysis_complete(self, analysis, token: int) -> None:
        context = self._critical_context
        if not context or token != context.get("token"):
            return
        if context.get("chart_capture_complete"):
            return super()._critical_analysis_complete(analysis, token)
        if context.get("chart_capture_started"):
            return

        chart_urls = [
            str(url or "").strip()
            for url in (analysis.chart_image_urls or [])
            if str(url or "").strip()
        ]
        ...
        if not chart_urls:
            self._show_guard_message(
                "Критический триггер",
                "Для критического триггера не задан URL изображения графика. "
                "Задача Redmine не подготовлена.",
            )
    ...
    def _critical_capture_next_chart(self, token: int) -> None:
        ...
        if index >= len(urls):
            context["chart_capture_complete"] = True
            self.logger.info(
                "Critical chart capture completed: id=%s attachments=%s",
                context["definition"].id,
                len(context.get("chart_attachments") or []),
            )
            super()._critical_analysis_complete(context["analysis"], token)
            return
    ...
    def _critical_chart_result(self, result, token: int, index: int) -> None:
        ...
        if not context or token != context.get("token") or index != context.get("chart_capture_index"):
            return
        payload = self._decode_chart_result(result)
        ...
        if payload.get("ok") and data_base64:
            try:
                ...
                decoded = b""
            if len(decoded) < 100 or not decoded.startswith(b"\x89PNG\r\n\x1a\n"):
                self._critical_chart_capture_failed(token, "captured_png_invalid")
                return

            graph_urls = list(context.get("chart_graph_urls") or [])
            context["chart_attachments"].append(
                {
                    "filename": _critical_attachment_filename(context["item"], index),
        ...
        if attempt <= len(CRITICAL_CHART_RETRY_DELAYS_MS):
            delay = CRITICAL_CHART_RETRY_DELAYS_MS[attempt - 1]
    ...
    def _open_critical_redmine_dialog(self, token: int) -> None:
        ...
        try:
            ...
            description = _critical_redmine_description(context)
            default_params = {
            ...
            self.logger.info(
                "Critical Redmine dialog opened with attachments: id=%s attachments=%s",
                context["definition"].id,
                len(attachments),
            )
        except Exception:
            ...
            self._show_guard_message(
                "Redmine",
                "Ошибка подготовки критической задачи Redmine с изображениями. "
    ...
    def _finish_critical_flow(self, token: int | None = None) -> None:
        self._cleanup_critical_chart_view()
        super()._finish_critical_flow(token)

    def hideEvent(self, event):
        # Leaving the Live Zabbix page cancels only an unfinished hidden-history
        # or chart read. A Redmine dialog or already-started acknowledgement may finish.
        if self._critical_history_view is not None or self._critical_chart_view is not None:
