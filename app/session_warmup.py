...
class SessionWarmupManager:
    ...
    def _on_html(self, html: str):
        ...
        if status == WarmupStatus.AUTH_REQUIRED and self._mode == MODE_SILENT and self.settings.get("silent_autologin", True):
            if not self._has_credentials_for(self._current):
                reason = "missing_credentials"
                self.logger.warning("Session warmup missing credentials: system=%s", self._current.system)
            elif self._try_silent_auth(url, html, reason):
                return
    ...
    def _try_silent_auth(self, url: str, html: str, reason: str) -> bool:
        ...
        if not script:
            return False
        self.logger.info("Session warmup silent autologin started: system=%s", target.system)
        self._view.page().runJavaScript(script, lambda _ok: None)
        return True

    def _has_credentials_for(self, target: WarmupTarget | str) -> bool:
        credentials = self._credentials_for(target)
